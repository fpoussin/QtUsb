...
qint32 QUsbDevice::read(QByteArray* buf, quint32 len)
{
    ...
    *buf = mReadBuffer;
    ...
    while (timer.elapsed() < mTimeout && (qint32)len - read_total > 0) {
        rc = libusb_bulk_transfer(mDevHandle, mConfig.readEp, (uchar*)(mReadBuffer.data() + read_total), mReadBufferSize, &read_bytes, 10);
    ...
    if (mDebug && timer.elapsed() >= mTimeout)
        qDebug("USB Timeout!");
...
qint32 QUsbDevice::write(const QByteArray* buf, quint32 len)
{
    ...
    while (timer.elapsed() < mTimeout && len-sent > 0) {
        rc = libusb_bulk_transfer(mDevHandle, (mConfig.writeEp), (uchar*)buf->data(), len, &sent_tmp, mTimeout);
        if (rc == LIBUSB_ERROR_PIPE) {
            libusb_clear_halt(mDevHandle, mConfig.readEp);
        }
        sent += sent_tmp;
